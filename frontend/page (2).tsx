...
import type { PredictionRow, VolatilityRow, FeatureImportanceRow, SignalsResponse, ForecastResponse } from "@/lib/api";
...
import {
  Zap, ArrowUpRight, ArrowDownRight, Database,
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [volatility, setVolatility] = useState<VolatilityRow[]>([]);
  const [threshold, setThreshold] = useState<number>(0);
  const [features, setFeatures] = useState<FeatureImportanceRow[]>([]);
  ...
  if (error) return <Shell><ErrorBox message={`Cannot reach API: ${error} — ensure the FastAPI backend is running.`} /></Shell>;
  if (!signals || !forecastData) return <Shell><div className="flex items-center justify-center h-64 text-[#4a6fa5]">Loading live data...</div></Shell>;

  const l  = signals.latest;
  const vt = signals?.vol_threshold ?? 0;
    ...
    <Shell rate={l.rate}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          />
          <StatCard label="1d Signal" value={forecastData.forecasts[0]?.signal ?? "—"} sub="Model output" icon={Activity} accent="#6366f1" />
          <StatCard
          ...
          />
          <StatCard label="Risk Level" value={l.high_volatility_flag ? "HIGH RISK" : "LOW RISK"} sub="Composite score" icon={Shield} accent="#8b5cf6" />
        </div>
